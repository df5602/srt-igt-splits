            ...
            splits: SplitsV1 {
                splits: splits
                    ...
                    .map(|s| SplitV1 {
                        name: s.name.clone(),
                        percent: s.percent,
                        duration: s.time.map(|duration| HmsDuration(duration)),
                    })
                    .collect(),
            },
        }
...
fn from_v1(file_v1: SplitsFileV1, path: &Path) -> Splits {
    let splits = file_v1
        ...
        .map(|s| Split {
            name: s.name,
            percent: s.percent,
            time: s.duration.map(|duration| duration.0),
            history: Vec::new(),
        })
        .collect();

    Splits::create(path.to_path_buf(), splits)
}
...
mod tests {
    ...
    fn save_then_load_round_trip() -> anyhow::Result<()> {
        ...
        let original_splits = Splits::create(
            file_path.clone(),
            vec![
                Split {
